DROP VIEW public.vw_room_type_availability;
...
CREATE OR REPLACE VIEW public.vw_room_type_availability
 ...
 SELECT a.room_type_id,
   ...
   FROM room_type_date_allotment a
     ...
     LEFT JOIN ( SELECT generate_series(booking.checkin_date::timestamp with time zone, (booking.checkout_date - 1)::timestamp with time zone, '1 day'::interval)::date AS date,
          ...
          GROUP BY (generate_series(booking.checkin_date::timestamp with time zone, (booking.checkout_date - 1)::timestamp with time zone, '1 day'::interval)::date), booking_room.room_type_id) sd ON a.room_type_id = sd.room_type_id AND a.date = sd.date
