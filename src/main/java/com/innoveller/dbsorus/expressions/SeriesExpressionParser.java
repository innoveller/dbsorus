...
public class SeriesExpressionParser {
    ...
    public static List<String> generateSeries(String rangeClause) {
        ...
        if(parts.length == 2) {
            ...
            } else if(fromPart.contains("today") && toPart.contains("today")) {
                try {
                    LocalDate fromLocalDate = DateTimeExpressionParser.parseDate(fromPart);
                    LocalDate toLocalDate = DateTimeExpressionParser.parseDate(toPart);
                    List<String> serialValues = new ArrayList<>();
                    for (LocalDate d = fromLocalDate; !d.isAfter(toLocalDate); d = d.plusDays(1)) {
                        serialValues.add(d.format(DateTimeFormatter.ISO_DATE));
                    }
